...
final class WpdbImportRepository extends AbstractWpdbRepository implements ImportRepository
{
    ...
    {
        $table = $this->table(TableName::IMPORT_JOBS); $row = $this->database->fetchRow("SELECT import_job_id, event_id, import_status, source_filename, source_file_hash, total_rows, valid_rows, invalid_rows, applied_rows, failed_rows, worker_lease_token, worker_lease_expires_at FROM {$table} WHERE event_id = %d AND import_job_id = %d LIMIT 1 FOR UPDATE", [$scope->eventId, $jobId]);
    ...
    {
        $table = $this->table(TableName::IMPORT_JOBS); if ($this->database->execute("UPDATE {$table} SET import_status = %s, mapping_definition = %s, valid_rows = %d, invalid_rows = %d, warning_rows = %d, validated_at = %s, updated_at = %s WHERE event_id = %d AND import_job_id = %d AND import_status = %s", [ImportStatus::VALIDATED->value, $this->json($mapping), $validRows, $invalidRows, $warningRows, $this->time($now), $this->time($now), $job->eventScope->eventId, $job->jobId, ImportStatus::STAGED->value]) !== 1) throw new PersistenceException('import_validation_conflict');
        return new ImportJobRecord($job->jobId, $job->eventScope, ImportStatus::VALIDATED, $job->sourceFilename, $job->sourceFileHash, $job->totalRows, $validRows, $invalidRows, 0, 0);
    }
    ...
    {
        $table = $this->table(TableName::IMPORT_JOBS); $affected = $this->database->execute("UPDATE {$table} SET import_status = %s, worker_lease_token = %s, worker_lease_owner = %s, worker_lease_expires_at = %s, worker_heartbeat_at = %s, updated_at = %s WHERE event_id = %d AND import_job_id = %d AND import_status IN (%s, %s) AND (worker_lease_token IS NULL OR worker_lease_expires_at <= %s)", [ImportStatus::APPLYING->value, $token, $owner, $this->time($expiresAt), $this->time($now), $this->time($now), $scope->eventId, $jobId, ImportStatus::VALIDATED->value, ImportStatus::APPLYING->value, $this->time($now)]);
    ...
    public function reconcile(ImportJobRecord $job, string $token, DateTimeImmutable $now): ImportJobRecord
    {
        ...
        if ($this->database->execute("UPDATE {$jobs} SET import_status = %s, applied_rows = %d, failed_rows = %d, completed_at = {$completedSql}, worker_lease_token = NULL, worker_lease_owner = NULL, worker_lease_expires_at = NULL, worker_heartbeat_at = NULL, updated_at = %s WHERE event_id = %d AND import_job_id = %d AND import_status = %s AND worker_lease_token = %s", $parameters) !== 1) throw new PersistenceException('import_reconcile_conflict');
        return new ImportJobRecord($job->jobId, $job->eventScope, $status, $job->sourceFilename, $job->sourceFileHash, $job->totalRows, $job->validRows, $job->invalidRows, $applied, $failed);
    }
    ...
    private function job(array $row, EventScope $scope): ImportJobRecord { $status = ImportStatus::tryFrom((string) ($row['import_status'] ?? '')); if ($status === null || (int) ($row['event_id'] ?? 0) !== $scope->eventId) throw new PersistenceException('import_job_invalid'); return new ImportJobRecord((int) $row['import_job_id'], $scope, $status, (string) $row['source_filename'], (string) $row['source_file_hash'], (int) $row['total_rows'], (int) $row['valid_rows'], (int) $row['invalid_rows'], (int) $row['applied_rows'], (int) $row['failed_rows'], $row['worker_lease_token'] ?? null, $this->date($row['worker_lease_expires_at'] ?? null)); }
    /** @param array<string,mixed> $row */
