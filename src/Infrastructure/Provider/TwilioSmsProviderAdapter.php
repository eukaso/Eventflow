...
{
    public function __construct(private ProviderHttpClient $http,private string $accountSid,private string $authSecret,private string $sender,private string $webhookUrl,private ?string $authSid=null,private ?string $signatureToken=null){}
    ...
    public function send(ProviderDispatchMessage $m):ProviderSendResult
    {
        ...
        $authIdentity=$this->authSid??$this->accountSid;
        $r=$this->http->post('https://api.twilio.com/2010-04-01/Accounts/'.rawurlencode($this->accountSid).'/Messages.json',['authorization'=>'Basic '.base64_encode($authIdentity.':'.$this->authSecret),'content-type'=>'application/x-www-form-urlencoded'],$body);$p=json_decode($r->body,true);$sid=is_array($p)&&is_string($p['sid']??null)?$p['sid']:null;
