...
final readonly class ProviderAdapterFactory
{
    ...
    public function fromConstants():ProviderRuntimeConfiguration
    {
        ...
        $twilioCore=$this->values(['EVENTFLOW_TWILIO_ACCOUNT_SID','EVENTFLOW_TWILIO_AUTH_TOKEN','EVENTFLOW_TWILIO_WEBHOOK_URL']);$twilioSender=$this->value('EVENTFLOW_TWILIO_MESSAGING_SERVICE_SID')??$this->value('EVENTFLOW_TWILIO_FROM_NUMBER');
        $twilio=$twilioCore===null||$twilioSender===null?null:[$twilioCore[0],$twilioCore[1],$twilioSender,$twilioCore[2]];
        $senderValid=$twilioSender!==null&&(preg_match('/^MG[a-fA-F0-9]{32}$/',$twilioSender)||preg_match('/^\+[1-9][0-9]{7,14}$/',$twilioSender));
        if($twilio!==null&&preg_match('/^AC[a-fA-F0-9]{32}$/',$twilio[0])&&$senderValid&&strlen($twilio[1])>=16&&filter_var($twilio[3],FILTER_VALIDATE_URL)!==false&&str_starts_with($twilio[3],'https://'))$adapters[]=new TwilioSmsProviderAdapter($this->http,...$twilio);else $issues[]=$twilio===null?'twilio_not_configured':'twilio_configuration_invalid';
