...
use EventFlow\Application\Job\{JobRepository,JobRequest};
use EventFlow\Application\Provider\ProviderDispatchGate;

final readonly class CommunicationService implements TemplateCommands, CampaignCommands
{
    public function __construct(private CommunicationRepository $repository, private AuthorizationService $authorization, private IdempotencyService $idempotency, private AuditService $audit, private Clock $clock, private TemplateRenderer $renderer, private ?JobRepository $jobs = null, private ?ProviderDispatchGate $dispatchGate = null, private ?MessageGuestLinkIssuer $guestLinks = null) {}
    ...
    {
        return $this->idempotency->execute($principal,$scope,'campaign.queue',$idempotencyKey,['campaign_id'=>$campaignId],function()use($principal,$scope,$campaignId){$this->authorization->requireEventCapability($principal,$scope,Capability::QUEUE_CAMPAIGN);$campaign=$this->repository->lockCampaign($scope,$campaignId)??throw new CommunicationException('resource_not_found');if(!in_array($campaign->status,['draft','scheduled'],true))throw new CommunicationException('campaign_already_queued');$template=$this->repository->lockPublishedTemplate($scope,$campaign->templateId)??throw new CommunicationException('campaign_template_invalid');if($template->channel!==$campaign->channel)throw new CommunicationException('campaign_channel_invalid');$provider=$campaign->channel===CommunicationChannel::EMAIL?'brevo':'twilio';$this->dispatchGate?->assertEnabled($provider);$needsGuestLink=in_array('guest_link',$template->allowedFields,true);if($needsGuestLink&&$this->guestLinks===null)throw new CommunicationException('guest_page_url_not_configured');$recipients=$this->repository->resolveRecipients($scope,$campaign);$messages=[];foreach($recipients as $recipient){$logical=hash('sha256','campaign:'.$campaign->campaignId.':recipient:'.$recipient->identity());$mergeFields=$recipient->mergeFields;if($needsGuestLink)$mergeFields['guest_link']=TemplateRenderer::PREVIEW_GUEST_LINK;$subject=$template->subject===null?null:$this->renderer->render($template->subject,$template->allowedFields,$mergeFields);$body=$this->renderer->render($template->body,$template->allowedFields,$mergeFields);$plain=$template->plainText===null?null:$this->renderer->render($template->plainText,$template->allowedFields,$mergeFields);$message=$this->repository->createOrFindMessage($scope,$campaign,$recipient,$logical,$subject,$body,$plain,$this->clock->now());if($needsGuestLink){$mergeFields['guest_link']=$this->guestLinks->issue($principal,$scope,$recipient->invitationId,$message->messageId,$campaign->purpose);$subject=$template->subject===null?null:$this->renderer->render($template->subject,$template->allowedFields,$mergeFields);$body=$this->renderer->render($template->body,$template->allowedFields,$mergeFields);$plain=$template->plainText===null?null:$this->renderer->render($template->plainText,$template->allowedFields,$mergeFields);$message=$this->repository->finalizeMessageContent($scope,$message->messageId,$subject,$body,$plain,$this->clock->now());}$messages[]=$message;if($this->jobs!==null)$this->jobs->enqueue(JobRequest::create($scope,'message.delivery.send',1,['message_id'=>$message->messageId,'provider'=>$provider],[Capability::QUEUE_CAMPAIGN],$this->clock->now(),40,5,'message:'.$message->messageId.':send'),$this->clock->now());}$this->repository->freezeQueued($scope,$campaign,count($messages),$this->clock->now());$this->audit->recordRequired(new AuditEvent($principal,$scope,AuditAction::CAMPAIGN_QUEUED,AuditEntityType::CAMPAIGN,$campaign->campaignId,after:['purpose'=>$campaign->purpose->value,'audience_mode'=>$campaign->audienceMode->value,'recipient_count'=>count($messages)]));$result=new CampaignQueueResult($campaign->campaignId,count($messages),$messages);return new IdempotentOperationResult(new IdempotencyResultReference('campaign',$campaign->campaignId,202),$result);});
    }
