...
final readonly class ImportNormalizer
{
    ...
    public function normalize(array $raw, ImportMapping $mapping): array
    {
        ...
        $capacity = $capacityRaw === null || $capacityRaw === '' ? 1 : filter_var($capacityRaw, FILTER_VALIDATE_INT, ['options' => ['min_range' => 1, 'max_range' => 65535]]);
        if ($capacity === false) $errors[] = 'capacity_invalid';
